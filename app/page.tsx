...
const Pages = () => {
  ...
  useEffect(() => {
    const applyFilters = (measure: Blueprint) => {
      ...
      const focusMatch =
        ...
        activeFilters.focuses.some((focus) => measure.focuses.includes(focus));
  ...
  const createBookmark = (name: string) => {
    ...
    if (!bookmarks.some((bookmark) => bookmark.name === name)) {
      setBookmarks((prev) => [...prev, { name, measures: [] }]);
    }
  };
    ...
    <div className='d-flex flex-column flex-grow-1'>
      ...
      <div className='app flex-grow-1'>
        ...
        <div className='main-content'>
          ...
          <MeasuresGrid
            blueprints={filteredMeasures}
            bookmarks={bookmarks}
            onAddToBookmark={addMeasureToBookmark}
          />
        </div>
      </div>
      <Footer />
